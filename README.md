...
- **OS → Application**: **[RUNBOOK.md](RUNBOOK.md)** — the default install path, blank eMMC to a ready printer: Sovobian image, offline preseed, KIAUH stack, plugins, config restore, verification. **[EXTRAS.md](EXTRAS.md)** — optional add-ons proven on this machine: USB WiFi dongle for a dead onboard radio, BTT SFS V2.0 wiring on the DET ports, the backup pair that makes kernel experiments reversible.
