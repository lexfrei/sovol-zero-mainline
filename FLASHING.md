...
- Even *replacing* Katapult itself doesn't strictly need SWD: a Katapult **deployer** flashed through the stock bootloader over USB installs mainline Katapult in place ([asnajder/zero-config](https://github.com/asnajder/zero-config) ships one). SWD stays the recovery path for a bootloader that is already corrupt.
- For an exact return to stock, [asnajder/zero-config](https://github.com/asnajder/zero-config) publishes ST-LINK-flashable vendor recovery images for the mainboard (including its Katapult), the toolhead, and the chamber MCU.
