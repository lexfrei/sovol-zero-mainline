...
Target `stm32f1x`. Header: the 4-pin `5V3 IO CK G` near the F103 (see [the toolhead pinout]({{< relref "/zero/hardware/toolhead" >}})). Disconnect the head from the printer — the probe's 3.3 V powers the MCU (a blue LED confirms power).
...
- In practice the H750 is flashed over **USB-Katapult** (see [MCU migration]({{< relref "mcu-migration" >}})), not SWD. Reserve SWD for the case where the app is corrupted and USB-Katapult can no longer be entered.
- Even *replacing* Katapult itself doesn't strictly need SWD: a Katapult **deployer** (build it from source — see [Building the firmware]({{< relref "build" >}})) flashed through the stock bootloader over USB installs mainline Katapult in place. SWD stays the recovery path for a bootloader that is already corrupt.
- For an exact return to stock, your **own SWD dump** (taken before you erased anything) is the rollback. If you never took one and you're stuck, [asnajder/zero-config](https://github.com/asnajder/zero-config) publishes ST-LINK-flashable vendor recovery images for the mainboard (including its Katapult), the toolhead, and the chamber MCU — but those are third-party prebuilts of unverifiable provenance, so treat them as an emergency fallback, not a routine source.
